...
paData <- NoControlData[grepl("G.a", NoControlData$isolate),]
piData <- NoControlData[grepl("G.i", NoControlData$isolate),]
puuData <- NoControlData[grepl("G.uu", NoControlData$isolate),]
...
capture.output({
  cat("Summary of ANOVA for lesionPercent:\n")
  print(summary(aov(lesionPercent ~ pedigree + repetition + block, paData)))
  
  cat("\n\nSummary of ANOVA for percentLengthPedByIsolate:\n")
  print(summary(aov(percentLengthPedByIsolate ~ pedigree + repetition + block, paData)))
  
  cat("\n\nSummary of ANOVA for percentMassPedByIsolate:\n")
  print(summary(aov(percentMassPedByIsolate ~ pedigree + repetition + block, paData)))
}, file = anovaFile)
...
LengthAnova <- lmer(percentLengthPedByIsolate_transformed ~ isolate * pedigree + repetition + (1|block), dataForTransformation)
summary(LengthAnova)
Anova(LengthAnova, Type = "III")
...
leveneTest(lesionPercent ~ isolate * pedigree + repetition, dataForTransformation) # if the p-value is below 0.05, we should probably transform it...
# BUT, if all other normality tests are greater than 0.05, this 
# becomes subjective... we don't have to. 
...
########################################################################################################################################################################################
########################################################################################################################################################################################
paDataTransformed <- dataForTransformation[grepl("G.a", dataForTransformation$isolate),]
piDataTransformed <- dataForTransformation[grepl("G.i", dataForTransformation$isolate),]
puuDataTransformed <- dataForTransformation[grepl("G.uu", dataForTransformation$isolate),]
########################################################################################################################################################################################
...
summary(aov(lesionPercent_transformed ~ pedigree + repetition + block, paDataTransformed))
summary(aov(percentLengthPedByIsolate_transformed ~ pedigree + repetition + block, paDataTransformed))
summary(aov(percentMassPedByIsolate_transformed ~ pedigree + repetition + block, paDataTransformed))
...
summary(aov(lesionPercent_transformed ~ pedigree + repetition + block, piDataTransformed))
summary(aov(percentLengthPedByIsolate_transformed ~ pedigree + repetition + block, piDataTransformed))
summary(aov(percentMassPedByIsolate_transformed ~ pedigree + repetition + block, piDataTransformed))
...
summary(aov(lesionPercent_transformed ~ pedigree + repetition + block, puuDataTransformed))
summary(aov(percentLengthPedByIsolate_transformed ~ pedigree + repetition + block, puuDataTransformed))
summary(aov(percentMassPedByIsolate_transformed ~ pedigree + repetition + block, puuDataTransformed))
