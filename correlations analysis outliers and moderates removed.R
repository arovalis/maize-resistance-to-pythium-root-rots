...
filteredData <- data %>%
  filter(pedigree %in% c("CML103", "CML277","H100"))


ogData <- filteredData[!grepl("Standard", filteredData$isolate),] #Remove the control groups, use this going forward.
...
########################################################################## WARM
warmExpData <- ogData[grepl("multiWarm", ogData$experimentType),] # Filtered to just use warm experiments
...
filteredData <- warmExpData %>%
  ...
  filter(
    lesionPercent <= meanLesionPercent + 2.01 * sdLesionPercent,
    percentLengthPedByIsolate <= meanPercentLength + 2.01 * sdPercentLength,
    percentMassPedByIsolate <= meanPercentMass + 2.01 * sdPercentMass
  )
...
ggsave(path = graphDir, filename, width = ChartWidth*2, height = ChartHeight*2, units = "px", dpi = 300, bg = "#FFFEFE", limitsize = FALSE)
filename2 <- paste0("Warm Experiments Disease between Guu and Ga.png", sep = "") 
ggsave(path = graphDir, filename2, width = ChartWidth*2, height = ChartHeight*2, units = "px", dpi = 300, bg = "#FFFEFE", limitsize = FALSE)
...
########################################################################## COLD
##### By Maize Line ###########################################################
coldExpData <- ogData[grepl("multiCold", ogData$experimentType),] # Filtered out experiments that just used Guu
...
corFiltData <- ogData[!grepl("singleSpecies", ogData$experimentType),] # Filtered out experiments that just used Guu
